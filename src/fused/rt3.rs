...
impl Inst {
    pub fn add<S>(result: Register, lhs: Register, rhs: S) -> Self
    where
        S: Into<Source>,
    {
        Self::Add(AddInst {
    ...
    pub fn sub<S>(result: Register, lhs: Register, rhs: S) -> Self
    where
        S: Into<Source>,
    {
        Self::Sub(SubInst {
    ...
    pub fn mul<S>(result: Register, lhs: Register, rhs: S) -> Self
    where
        S: Into<Source>,
    {
        Self::Mul(MulInst {
    ...
    ( $( $inst_name:ident($op_name:ident) ),* $(,)? ) => {
        $(
            ...
            impl Execute for $inst_name {
                fn execute(&self, context: &mut Context) -> Outcome {
                    let lhs = context.get_reg(self.lhs);
